...
# --- CÁLCULO DE BATERÍA ---
def obtener_porcentaje_bat():
    v = sensores.leer_bateria()
    if v < 2.0: return 0 # Evitar lecturas basura si está desoldada
    pct = (v - 3.2) / (4.2 - 3.2) * 100
...
def dibujar_barra_estado():
    ...
    pct_bat = obtener_porcentaje_bat()
    
    oled.fill_rect(0, 0, 128, 10, 0)
    oled.text(f"{st_wifi} v{VERSION_ACTUAL}", 0, 0)
    oled.text(f"{pct_bat}%", 100, 0) # Solo el número y el símbolo %
...
def app_modo_usb():
    ...
    while modo_activo:
        ...
        if poller.poll(500): 
            ...
            if cmd == "GET_DATA":
                print(ujson.dumps({"v_bat": obtener_porcentaje_bat(), "temp": sensores.leer_termistor()}))
    ...
    if pantalla_encendida:
        ...
        if not modo_activo:
            ...
            if sel == "GONIOMETRO": app_goniometro_master()
                ...
                oled.text(f"CARGA: {obtener_porcentaje_bat()}%", 25, 30); oled.show(); time.sleep(0.5)
            elif sel == "SYNC NUBE":
